...
#~~~
    ...
    for t in range(hyper_params["num-steps"]):
        ...
        if sample > eps_threshold:
            ...
            action = np.random.choice(action_list)#action_list[env.action_space.sample()%6] 
...
if __name__ == '__main__':
    ...
    hyper_params = {
        ...
        "num-steps": int(7e6),  # total number of steps to run the environment for
        ...
        "eps-end": 0.03,  # e-greedy end threshold
    ...
    eval_seeds = [1, 2, 3]
    arr = []    
    for i in range(hyper_params["num-steps"]):
        arr.append(np.random.randint(50))
    ...
    env = ObstacleTowerEvaluation(env, arr)
    ...
    #env = ObstacleTowerEvaluation(env, arrr)
    #episode_rew = run_episode(env)
